"""
The Asset Studio's data contracts.

These lived in `schemas/campaign.py` until that name was claimed by campaign
CRUD — two entirely different things that happened to share a filename. The
studio's models moved here rather than the other way round: campaign management
is the shared vocabulary of the app, and the studio is one stage inside it.

Nothing outside `app.services.studio` should import these directly. The team's
`schemas/campaign_dto.py` is the contract between stages, and `dto_bridge`
translates. These are internal.

Originally: shared data contracts between the 3 campaign agents.
