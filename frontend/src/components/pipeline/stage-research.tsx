...
const CitationNavigationContext = React.createContext<(number: number) => void>(() => undefined);
...
function InlineCitations({ evidence, citationNumbers }: { evidence: ResearchEvidence[]; citationNumbers: Map<string, number> }) {
  const openCitation = React.useContext(CitationNavigationContext);
  const citations = [...new Set(evidence.flatMap(item => item.source_url ? [citationNumbers.get(item.source_url)] : []).filter((number): number is number => number !== undefined))];
  if (!citations.length) return null;
  return <sup className="ml-1 whitespace-nowrap">{citations.map(number => <a key={number} href={`#nguon-${number}`} onClick={(event) => { event.preventDefault(); openCitation(number); }} title={`Xem nguồn ${number}`} className="text-[#35ea52] hover:underline font-mono text-[0.72em]">[{number}]</a>)}</sup>;
}
...
export const StageResearch: React.FC<Props> = ({ plan, isLoading, error, onRetry }) => {
  ...
  const openCitation = React.useCallback((number: number) => {
    setTab("sources");
    window.requestAnimationFrame(() => document.getElementById(`nguon-${number}`)?.scrollIntoView({ behavior: "smooth", block: "start" }));
  }, []);
  ...
  return <CitationNavigationContext.Provider value={openCitation}><div className="space-y-6 h-full flex flex-col animate-in fade-in duration-500">
  ...
  </div></CitationNavigationContext.Provider>;
};
